			...
			<div id="main">
				...
				<table class="bordered">
					...
					<?php
						...
						// Siapkan query
						$sql = $search ? 
							"SELECT * FROM tbl_normalisasi WHERE id_kriteria LIKE ?" :
							"SELECT * FROM tbl_normalisasi";
						...
						if ($stmt = mysqli_prepare($koneksi, $sql)) {
							...
							while ($row = mysqli_fetch_assoc($result)) {
								echo "<tr>
									<td align='center'>" . htmlspecialchars($row['id_normalisasi']) . "</td>
									<td><a href='detail-kriteria.php?kode=" . urlencode($row['id_kriteria']) . "'>" . htmlspecialchars($row['id_kriteria']) . "</a></td>
									<td>" . htmlspecialchars($row['n_penghasilan_ortu']) . "</td>
									<td>" . htmlspecialchars($row['n_nilai_ipk']) . "</td>
									<td>" . htmlspecialchars($row['n_semester']) . "</td>
									<td>" . htmlspecialchars($row['n_tanggungan_ortu']) . "</td>
									<td>" . htmlspecialchars($row['n_saudara_kandung']) . "</td>
									<td align='center'>
										<a href='detail-normalisasi.php?kode=" . urlencode($row['id_normalisasi']) . "'>Detail</a> |
										<a href='form-normalisasi.php?kode=" . urlencode($row['id_normalisasi']) . "'>Ubah</a> |
										<a href='hapus-normalisasi.php?kode=" . urlencode($row['id_normalisasi']) . "'>Hapus</a>
									</td>
								</tr>";
							}
